...
<div id="vue-galeri-edit" class="container-fluid pt-3 pb-5 px-md-4">
    <div class="mb-4">
        ...
        <h3 class="fw-bold mb-1" style="color: #0A2540;">
            <i class="fas fa-edit me-2" style="color: #FFC973;"></i> Edit Foto Galeri
        </h3>
    </div>
                ...
                <div class="admin-card-body p-4 p-md-5">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="mb-4">
                            ...
                            <input type="text" name="judul" class="form-control form-control-lg" value="<?= htmlspecialchars($data['judul']) ?>" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="kategori" class="form-select form-select-lg" required>
                                ...
                                <option value="Pemancingan" <?= $data['kategori'] == 'Pemancingan' ? 'selected' : '' ?>>Pemancingan</option>
                                <option value="Fasilitas" <?= $data['kategori'] == 'Fasilitas' ? 'selected' : '' ?>>Fasilitas</option>
                                <option value="Suasana" <?= $data['kategori'] == 'Suasana' ? 'selected' : '' ?>>Suasana</option>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-semibold">Ganti Gambar (Opsional)</label>
                            <input type="file" name="gambar" @change="previewImage" class="form-control form-control-lg" accept="image/jpeg,image/jpg,image/png">
                            <small class="text-muted mt-2 d-block">Biarkan kosong jika tidak ingin mengganti foto saat ini.</small>
                        </div>
                        <div class="mb-5">
...
<script>
    ...
    createApp({
        setup() {
            // Vue mengambil gambar lama dari server (PHP) sebagai nilai awal
            const imageUrl = ref('<?= BASE_URL . '/' . $data['gambar_url'] ?>');
            
            // Jika admin memilih file baru, Vue akan mengganti tampilannya
